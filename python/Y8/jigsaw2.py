...
def encipher(msg,key):
    ...
    # set up the table
    k, m = len(key), len(msg)
    t = [msg[i:i + k] if i + k < m else msg[i:] for i in range(0, m, k)]
    # encipher
    cipher = ''
    for y in [key.index(x + 1) for x in range(k)]:
        cipher += ''.join(row[y] for row in t if y < len(row))
    return cipher
    
def decipher(msg, key):
    """Takes the message and the key and prints the deciphered message."""
    lKey, lMsg = len(key), len(msg)
    rows = ceil(lMsg / lKey)
    fullCols = lKey - (rows * lKey - lMsg)
    table, count = [''] * lKey, 0
    # reassemble the table
    for num in range(1, lKey + 1):
        ind = key.index(num)
        end = count + rows
        if ind >= fullCols:
            end -= 1
